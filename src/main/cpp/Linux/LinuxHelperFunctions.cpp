		...
		if (directoryEntry->d_type == DT_DIR)
		{
			...
			if ((directoryEntry->d_name[0] != '.') && (strcmp(directoryEntry->d_name, "virtual") != 0))
			{
				...
				if ((strlen(directoryEntry->d_name) > 3) && (directoryEntry->d_name[0] == 't') && (directoryEntry->d_name[1] == 't') && (directoryEntry->d_name[2] == 'y'))
				{
					...
					if (friendlyName.empty())		// Must be a physical platform port
					{
						// Ensure that the platform port is actually open
						struct serial_struct serialInfo;
						int fd = open((std::string("/dev/") + std::string(directoryEntry->d_name)).c_str(), O_RDWR | O_NONBLOCK | O_NOCTTY);
						if (fd >= 0)
						{
							if ((ioctl(fd, TIOCGSERIAL, &serialInfo) == 0) && (serialInfo.type != PORT_UNKNOWN))
								comPorts.push_back(std::make_pair(std::string("/dev/") + std::string(directoryEntry->d_name),
										std::string("Physical Port ") + std::string(directoryEntry->d_name).substr(3)));
							close(fd);
						}
					}
					else
